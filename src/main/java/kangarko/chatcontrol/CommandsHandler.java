...
public class CommandsHandler implements CommandExecutor {
	...
	private void handleCommand(CommandSender sender, String[] args) throws InsufficientPermissionException {
		...
		/** LIST COMMAND */
			...
			Common.tell(sender, " ", "&3  ChatControl &f(v" + ChatControl.instance().getDescription().getVersion() + ")", "&2  [] &f= optional arguments (use only 1 at once)", "&6  <> &f= required arguments", " ", "  &f/chc mute &9[-silent] [-anonymous] &2[reason] &e- Chat (un)mute.", "  &f/chc clear &9[-s] [-a] [-console] &2[reason] &e- Chat clear.", "  &f/chc fake &6<join/leave> &2[name] &e- Fake join/quit messages.", "  &f/chc reload &e- Reload configuration.", "  &f/chc list &e- Command list.");
		}
	...
	private void checkPerm(CommandSender sender, String perm) throws InsufficientPermissionException {
		if (sender instanceof Player && !Common.hasPerm(sender, perm)) throw new InsufficientPermissionException(perm);
	}
}
