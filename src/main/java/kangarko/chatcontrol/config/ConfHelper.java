...
public abstract class ConfHelper {
	...
	private static <T> void validate(String path, T def) {
		if (file == null) throw new IllegalStateException("Inbuilt config doesn't contain " + def.getClass().getSimpleName() + " at \"" + path + "\". Is it outdated?");
	...
	public static class CasusHelper {
		...
		protected CasusHelper(String raw) {
			...
			if (values.length != 3) throw new RuntimeException("Malformed type, use format: 'second, seconds' OR 'sekundu, sekundy, sekund' (if your language has it)");
		...
		public String formatNumbers(long count) {
			if (count == 1) return akuzativSg;
			if (count > 1 && count < 5) return nominativPl;
